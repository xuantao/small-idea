...
void CSharpExporter::OnBegin(const IScope* global, const std::string& file)
{
    _global = global;
    _file = file;
...
void CSharpExporter::OnFileBegin(const std::string& file)
{
}

void CSharpExporter::OnFileEnd()
{
}

void CSharpExporter::OnNsBegin(const std::string& name)
{
    int tab = _tab;
    _OUTS_ <<
...
void CSharpExporter::OnInclude(const std::string& file)
{
}

void CSharpExporter::OnVariate(const IVariate* var)
{
    CppVarData data;
...
void CSharpExporter::OnType(const IType* type)
{
    if (_lastIsVar)
    {
        _OUTS_ << std::endl;
...
bool CSharpExporter::Declare(const IEnumType* ty)
{
    int tab = _tab;
...
bool CSharpExporter::Declare(const IStructType* sType)
{
    int tab = _tab;
...
bool CSharpExporter::HeaderDeclare()
{
    int tab = _tab;
    if (!_enums.empty())
    {
...
bool CSharpExporter::EnumData(const IEnumType* eType, int tab)
{
...
bool CSharpExporter::EnumHelperDeclare(const IEnumType* eType, int tab)
{
    std::vector<std::string> path = utility::Absolute(eType);
...
bool CSharpExporter::EnumHelper(const IEnumType* eType, int tab)
{
    std::vector<std::string> path = utility::Absolute(eType);
    std::string tyName = utility::Contact(path, "::");
...
void CSharpExporter::TabDeclare(const IStructType* sType, int tab)
{
...
void CSharpExporter::TabLoader(const IStructType* sType, int tab)
{
...
void CSharpExporter::TabLoaderStatic(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
    _OUTS_ <<
...
void CSharpExporter::TabLoadDetail(const IStructType* sType, int tab)
{
    if (sType->Inherited())
    {
...
void CSharpExporter::TabLoadVar(const IVariate* var, const IRawType* rType, int tab)
{
    switch (rType->RawCat())
    {
...
void CSharpExporter::TabLoadVar(const IVariate* var, const IEnumType* eType, int tab)
{
...
void CSharpExporter::TabLoadVar(const IVariate* var, const IStructType* rType, int tab)
{
    _OUTS_ <<
        _TAB_EX_(0) << "sLoad(iter, out." << var->Name() << ");" << std::endl;
}

void CSharpExporter::TabLoadVar(const IVariate* var, const IRawType* rType, int length, int tab)
{
    _TAB_ << "iter.MoveNext();" << std::endl;
...
void CSharpExporter::TabLoadVar(const IVariate* var, const IEnumType* eType, int length, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(eType), "::");
    _OUTS_ <<
        _TAB_EX_(0) << "iter.MoveNext();" << std::endl <<
...
void CSharpExporter::TabLoadVar(const IVariate* var, const IStructType* rType, int length, int tab)
{
    _OUTS_ <<
...
void CSharpExporter::TabWriteHeader(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
    _OUTS_ <<
        _TAB_EX_(0) << "void WriteHeader(std::ostream& stream, const " << tyName << "& def)" << std::endl <<
...
void CSharpExporter::TabWriter(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
    _OUTS_ <<
        _TAB_EX_(0) << "void Write(std::ostream& stream, const " << tyName << "& data)" << std::endl <<
...
void CSharpExporter::TabWriterStatic(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
    _OUTS_ <<
...
void CSharpExporter::TabWriterDetail(const IStructType* sType, int tab)
{
    const IVarSet* vars = sType->OwnScope()->VarSet();
    if (sType->Inherited())
    {
...
void CSharpExporter::JsonDeclare(const IStructType* sType, int tab)
{
    std::vector<std::string> path = utility::Absolute(sType);
    std::string tyName = utility::Contact(path, "::");
...
void CSharpExporter::JsonLoader(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
...
void CSharpExporter::JsonLoaderStatic(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
    _OUTS_ <<
...
void CSharpExporter::JsonLoadVar(const IVariate* var, const IRawType* rType, int tab)
{
    _OUTS_ <<
...
void CSharpExporter::JsonLoadVar(const IVariate* var, const IEnumType* rType, int tab)
{
...
void CSharpExporter::JsonLoadVar(const IVariate* var, const IStructType* rType, int tab)
{
    _OUTS_ <<
        _TAB_EX_(0) << "if (node.isMember(\"" << var->Name() << "\"))" << std::endl <<
        _TAB_EX_(0) << "{" << std::endl <<
...
void CSharpExporter::JsonLoadVar(const IVariate* var, const IRawType* rType, int length, int tab)
{
    _OUTS_ <<
        _TAB_EX_(0) << "if (node.isMember(\"" << var->Name() << "\"))" << std::endl <<
        _TAB_EX_(0) << "{" << std::endl <<
...
void CSharpExporter::JsonLoadVar(const IVariate* var, const IEnumType* rType, int length, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(rType), "::");
    _OUTS_ <<
        _TAB_EX_(0) << "if (node.isMember(\"" << var->Name() << "\"))" << std::endl <<
...
void CSharpExporter::JsonLoadVar(const IVariate* var, const IStructType* rType, int length, int tab)
{
    _OUTS_ <<
        _TAB_EX_(0) << "if (node.isMember(\"" << var->Name() << "\"))" << std::endl <<
        _TAB_EX_(0) << "{" << std::endl <<
...
void CSharpExporter::JsonWriter(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
    _OUTS_ <<
        _TAB_EX_(0) << "void Write(std::ostream& stream, const " << tyName << "& data)" << std::endl <<
...
void CSharpExporter::JsonWriterStatic(const IStructType* sType, int tab)
{
    std::string tyName = utility::Contact(utility::Absolute(sType), "::");
    _OUTS_ <<
...
void CSharpExporter::GetDepends(const IStructType* sType, std::vector<const IStructType*>& deps)
{
